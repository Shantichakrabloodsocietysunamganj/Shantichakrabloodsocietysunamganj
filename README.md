...
- **রক্তের অনুরোধ** — জরুরি রক্তের প্রয়োজন পোস্ট করার ফর্ম; কোনো admin approval ছাড়াই সাথে সাথে লাইভ
- **অনুরোধকারী ফলো-আপ** — যে ব্রাউজার থেকে অনুরোধ করা হয় শুধু সেখানেই নির্ধারিত সময়ে “রক্ত পেয়েছেন?” নোটিশ আসে। “পেয়েছি” দিলে অনুরোধ সম্পন্ন হয়, “এখনো পাইনি” দিলে লাইভ থাকে এবং ২৪ ঘণ্টা পরে আবার জানায়। বিদ্যমান Supabase project-এ একবার `supabase/requester-followup.sql` চালাতে হবে।
...
- **🔴 রক্তপ্রার্থী লাইভ সিস্টেম** (`/blood-seekers`) — যাঁরা রক্তের অনুরোধ দেন তাঁরাও **রক্তদাতার মতোই কার্ডে** দেখা যান (avatar, গ্রুপ ব্যাজ, লাইভ স্ট্যাটাস ডট, কল/WhatsApp)। Supabase **Realtime** দিয়ে নতুন অনুরোধ পোস্ট হওয়ামাত্র **সাথে সাথে** সব ইউজারের স্ক্রিনে যুক্ত হয় — সাথে "এইমাত্র এলো" হাইলাইট, লাইভ পপ-আপ অ্যালার্ট ও নোটিফিকেশন সাউন্ড। Realtime বন্ধ থাকলে ৪৫ সেকেন্ডের polling fallback কাজ করে।
  > ⚠️ **চালু করতে:** Supabase Dashboard → SQL Editor-এ `supabase/enable-realtime.sql` ফাইলটি পেস্ট করে Run করুন (একবারই)। বিকল্পভাবে Dashboard → Database → Replication → `supabase_realtime` → `blood_requests` টেবিল enable করলেও হবে। নতুন কোনো টেবিল/কলাম লাগবে না।

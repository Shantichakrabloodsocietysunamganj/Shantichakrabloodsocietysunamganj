...
-- =====================================================================
-- 22) REQUESTER FOLLOW-UP — private device ownership + secure status RPC
-- Full standalone migration: supabase/requester-followup.sql
-- =====================================================================
...
create or replace function public.create_blood_request_with_access(p_request jsonb, p_token text)
returns uuid language plpgsql security definer set search_path = public, extensions as $$
declare
  v_id uuid := (p_request->>'id')::uuid;
  v_next_prompt timestamptz;
begin
  if length(coalesce(p_token, '')) < 32 then raise exception 'Invalid management token'; end if;
  v_next_prompt := greatest(now() + interval '12 hours', coalesce((p_request->>'next_prompt_at')::timestamptz, now() + interval '12 hours'));
  insert into public.blood_requests (
    ...
    patient_gender, disease, blood_component, request_type, status, requested_by, created_at
  ) values (
    v_id, p_request->>'patient_name', p_request->>'blood_group', (p_request->>'units_needed')::int,
    p_request->>'hospital', coalesce(nullif(p_request->>'district', ''), 'সুনামগঞ্জ'), p_request->>'upazila',
    (p_request->>'needed_date')::date, p_request->>'contact_name', p_request->>'contact_phone',
    nullif(p_request->>'message', ''), nullif(p_request->>'hemoglobin', ''), nullif(p_request->>'patient_age', '')::int,
    nullif(p_request->>'patient_gender', ''), nullif(p_request->>'disease', ''),
    coalesce(nullif(p_request->>'blood_component', ''), 'whole_blood'), 'normal', 'approved', auth.uid(),
...
create or replace function public.respond_to_blood_request(p_request_id uuid, p_token text, p_received boolean)
returns boolean language plpgsql security definer set search_path = public, extensions as $$
begin
  if not exists (
    select 1 from public.blood_request_access
  ...
  if p_received then
    update public.blood_requests set status = 'completed'
      where id = p_request_id and status in ('pending', 'approved');
    update public.blood_request_access set completed_at = now() where request_id = p_request_id;
  else
    update public.blood_request_access set next_prompt_at = now() + interval '24 hours' where request_id = p_request_id;
  end if;
  return true;
end;
$$;
...
create index if not exists blood_request_access_next_prompt_idx on public.blood_request_access (next_prompt_at) where completed_at is null;

    ...
    <div className="container-page py-10"><div className="mx-auto max-w-3xl">
      ...
      <form onSubmit={submit} className="mt-8 card p-6 sm:p-8">
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <Field label={en ? "Last Donation Date" : "সর্বশেষ রক্তদান"}><input type="date" className="input" value={form.last_donation_date} onChange={(e) => { const d = e.target.value; set("last_donation_date", d); const el = getEligibility(d || null, en); set("is_available", el.eligible); }} />{form.last_donation_date && (() => { const el = getEligibility(form.last_donation_date, en); return <p className={`mt-1 text-xs font-medium ${el.eligible ? "text-emerald-600" : "text-amber-600"}`}>{el.eligible ? (en ? "✓ Eligible to donate now." : "✓ আপনি এখন রক্তদানে প্রস্তুত।") : (en ? `⏳ ${el.daysRemaining} days remaining (eligible ${el.nextEligibleText}).` : `⏳ আর ${el.daysRemaining.toLocaleString("bn-BD")} দিন বাকি (${el.nextEligibleText})।`)}</p>; })()}</Field>
        </div>

...
export default function AdminPage() {
  ...
  async function deleteContact(id: string) {
    ...
    logActivity(tx("যোগাযোগ বার্তা মুছেছেন"));
    load();
  }
  ...
  async function bulkDeleteDonors() {
    ...
    if (!confirm(`${selectedDonors.size} ${tx("জন দাতা মুছে ফেলবেন?")}`)) return;
    setBulkBusy(true);
  ...
  async function bulkDeleteReqs() {
    ...
    if (!confirm(`${selectedReqs.size} ${tx("টি অনুরোধ মুছে ফেলবেন?")}`)) return;
    setBulkBusy(true);
  ...
  if (!authed)
    return (
      <div className="container-page py-20 text-center">
        ...
        <p className="mt-2 font-medium text-ink">{tx("এই পেজ শুধু অ্যাডমিনদের জন্য।")}</p>
        <Link href="/" className="btn-outline mt-4">{tx("হোমে ফিরুন")}</Link>
      </div>
    );
  ...
  return (
    <div className="container-page py-10">
      <header className="mb-8 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-display text-2xl font-extrabold tracking-tight text-ink">{role === "admin" ? tx("🛡️ অ্যাডমিন ড্যাশবোর্ড") : tx("🔹 মডারেটর ড্যাশবোর্ড")}</h1>
          <p className="text-sm text-ink/60">{tx("সমিতির সম্পূর্ণ ব্যবস্থাপনা এক জায়গায়।")}</p>
        </div>
        <div className="flex gap-2">
          <Link href="/" className="btn-outline">{tx("হোমে যান")}</Link>
          {role === "admin" && <Link href="/admin/settings" className="btn-primary">{tx("⚙️ সেটিংস")}</Link>}
        </div>
      </header>
      ...
      <nav className="mb-6 flex flex-wrap gap-2">
        {(["overview", "requests", "donors", "contacts"] as Tab[]).map((t) => (
          <button key={t} onClick={() => setTab(t)}
            ...
            {t === "overview" ? tx("📊 ওভারভিউ") : t === "requests" ? `${tx("📋 অনুরোধ")} (${requests.length})` : t === "donors" ? `${tx("🩸 দাতা")} (${donors.length})` : `${tx("✉️ বার্তা")} (${contacts.length})`}
          </button>
        ))}
      </nav>
      ...
      {tab === "overview" && (
        <>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <StatCard icon="🩸" label={tx("মোট রক্তদাতা")} value={stats.donors} color="bg-brand-50 text-brand-600" />
            <StatCard icon="📋" label={tx("মোট অনুরোধ")} value={stats.requests} color="bg-amber-50 text-amber-600" />
            <StatCard icon="⏳" label={tx("চলমান অনুরোধ")} value={stats.pending} color="bg-blood-50 text-blood-600" />
            <StatCard icon="✅" label={tx("সম্পন্ন")} value={stats.completed} color="bg-success-50 text-success-700" />
            <StatCard icon="✉️" label={tx("যোগাযোগ বার্তা")} value={stats.contacts} color="bg-violet-50 text-violet-600" />
            <StatCard icon="🙋" label={tx("স্বেচ্ছাসেবক")} value={stats.volunteers} color="bg-sky-50 text-sky-600" />
          </div>

          <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <Link href="/admin/settings" className="card-hover flex items-center gap-3 p-4 text-sm font-medium text-ink"><span className="text-xl">⚙️</span> {tx("ওয়েবসাইট সেটিংস")}</Link>
            <Link href="/admin/events" className="card-hover flex items-center gap-3 p-4 text-sm font-medium text-ink"><span className="text-xl">📅</span> {tx("ইভেন্ট")}</Link>
            <Link href="/admin/blog" className="card-hover flex items-center gap-3 p-4 text-sm font-medium text-ink"><span className="text-xl">📝</span> {tx("ব্লগ")}</Link>
            ...
            <Link href="/admin/committee" className="card-hover flex items-center gap-3 p-4 text-sm font-medium text-ink"><span className="text-xl">👥</span> {tx("কমিটি")}</Link>
            <Link href="/admin/volunteers" className="card-hover flex items-center gap-3 p-4 text-sm font-medium text-ink"><span className="text-xl">🙋</span> {tx("স্বেচ্ছাসেবক")}</Link>
            <Link href="/admin/broadcast" className="card-hover flex items-center gap-3 p-4 text-sm font-medium text-ink"><span className="text-xl">🔔</span> {tx("নোটিফিকেশন প্রেরণ")}</Link>
            <Link href="/admin/reports" className="card-hover flex items-center gap-3 p-4 text-sm font-medium text-ink"><span className="text-xl">📊</span> {tx("রিপোর্ট (PDF/CSV)")}</Link>
            <Link href="/admin/activity" className="card-hover flex items-center gap-3 p-4 text-sm font-medium text-ink"><span className="text-xl">📋</span> {tx("অ্যাক্টিভিটি লগ")}</Link>
            <Link href="/admin/donations" className="card-hover flex items-center gap-3 p-4 text-sm font-medium text-ink"><span className="text-xl">🩸</span> {tx("রক্তদান রেকর্ড")}</Link>
            <Link href="/admin/media-coverage" className="card-hover flex items-center gap-3 p-4 text-sm font-medium text-ink"><span className="text-xl">📰</span> {tx("মিডিয়া কভারেজ")}</Link>
            <Link href="/admin/donation-methods" className="card-hover flex items-center gap-3 p-4 text-sm font-medium text-ink"><span className="text-xl">💳</span> {tx("ডোনেশন মেথড")}</Link>
            <Link href="/admin/media" className="card-hover flex items-center gap-3 p-4 text-sm font-medium text-ink"><span className="text-xl">🖼️</span> {tx("মিডিয়া লাইব্রেরি")}</Link>
            ...
            <Link href="/admin/partners" className="card-hover flex items-center gap-3 p-4 text-sm font-medium text-ink"><span className="text-xl">🤝</span> {tx("পার্টনার")}</Link>
            ...
            <Link href="/admin/trash" className="card-hover flex items-center gap-3 p-4 text-sm font-medium text-ink"><span className="text-xl">🗑️</span> {tx("ট্র্যাশ")}</Link>
          ...
          <div className="card mt-6 p-6">
            <h3 className="mb-4 font-semibold text-ink">{tx("রক্তের গ্রুপ অনুযায়ী দাতা বিতরণ")}</h3>
            {(() => {
      ...
      {tab === "requests" && (
        <div className="space-y-3">
          <div className="flex justify-end gap-2">
            <button onClick={() => exportCSV("blood-requests.csv", requests)} className="btn-outline !py-2 text-xs">{tx("⬇️ CSV ডাউনলোড")}</button>
          </div>
          {/* Bulk action bar */}
          {selectedReqs.size > 0 && (
            <div className="flex items-center justify-between rounded-xl bg-brand-50 p-3">
              <span className="text-sm font-medium text-brand-700">{selectedReqs.size} {tx("টি নির্বাচিত")}</span>
              <div className="flex gap-2">
          ...
          <div className="flex items-center gap-2 px-1">
            ...
            <span className="text-sm text-ink/60">{tx("সব নির্বাচন করুন")}</span>
          </div>
          {requests.length === 0 ? <Empty text={tx("কোনো অনুরোধ নেই।")} /> : requests.map((r) => (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-ink/60">{r.hospital}, {r.upazila} • {r.units_needed} {tx("ইউনিট • 📞")} {r.contact_phone}</p>
                      </div>
                    </div>
        ...
        <div className="card overflow-hidden">
          ...
          {selectedDonors.size > 0 && (
            <div className="flex items-center justify-between border-b border-zinc-100 bg-brand-50 p-3">
              <span className="text-sm font-medium text-brand-700">{selectedDonors.size} {tx("জন নির্বাচিত")}</span>
          ...
          <div className="flex justify-end border-b border-zinc-100 p-3">
            <button onClick={() => exportCSV("donors.csv", donors)} className="btn-outline !py-1.5 text-xs">{tx("⬇️ CSV ডাউনলোড")}</button>
          </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-canvas text-left text-xs uppercase text-ink/50">
                <tr>
                  ...
                  <th className="p-3">{tx("নাম")}</th><th className="p-3">{tx("গ্রুপ")}</th><th className="p-3">{tx("এলাকা")}</th><th className="p-3">{tx("ফোন")}</th><th className="p-3">{tx("স্ট্যাটাস")}</th><th className="p-3"></th>
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-100">
                {donors.map((d) => (
                  <tr key={d.id} className={selectedDonors.has(d.id) ? "bg-brand-50/50" : ""}>
                    ...
                    <td className="p-3 font-medium text-ink">{d.full_name}{d.is_verified && <span title="verified"> ✓</span>}{!d.approved && <span className="ml-1 rounded bg-amber-100 px-1.5 py-0.5 text-[10px] font-semibold text-amber-700">{tx("⏳ অপেক্ষমাণ")}</span>}</td>
                    ...
                    <td className="p-3">{d.is_available ? <span className="text-success-700">{tx("প্রস্তুত")}</span> : <span className="text-ink/40">{tx("অনুপস্থিত")}</span>}</td>
...
function RequestActions({ req, onChanged }: { req: BloodRequest; onChanged: () => void }) {
  ...
  async function setStatus(s: string) { await supabase.from("blood_requests").update({ status: s }).eq("id", req.id); logActivity(tx("অনুরোধের স্ট্যাটাস পরিবর্তন"), `${req.patient_name} → ${s}`); onChanged(); }
  async function remove() { await supabase.from("blood_requests").update({ deleted_at: new Date().toISOString() }).eq("id", req.id); logActivity(tx("রক্তের অনুরোধ মুছেছেন"), req.patient_name); onChanged(); }
  return (
    <div className="mt-3 flex flex-wrap gap-2">
      {status !== "completed" && <button onClick={() => setStatus("completed")} className="btn-outline !px-3 !py-1.5 text-xs text-success-700">{tx("সম্পন্ন")}</button>}
      {status !== "cancelled" && <button onClick={() => setStatus("cancelled")} className="btn-ghost !px-3 !py-1.5 text-xs">{tx("বাতিল")}</button>}
      <button onClick={remove} className="btn-ghost !px-3 !py-1.5 text-xs text-blood-600">{tx("মুছুন")}</button>
    </div>
  );
}
...
function DonorActions({ donor, onChanged }: { donor: Donor; onChanged: () => void }) {
  ...
  async function toggleApproved() {
    ...
    logActivity(donor.approved ? tx("দাতা অনুমোদন বাতিল করেছেন") : tx("দাতা অনুমোদন করেছেন"), donor.full_name);
    onChanged();
  }
  async function remove() { await supabase.from("donors").update({ deleted_at: new Date().toISOString() }).eq("id", donor.id); logActivity(tx("রক্তদাতা মুছেছেন"), donor.full_name); onChanged(); }
  return (
    <div className="flex items-center justify-end gap-1">
      <button onClick={toggleApproved} className={donor.approved ? "btn-ghost !px-2 !py-1 text-xs text-success-700" : "btn-primary !px-2 !py-1 text-xs"}>
        {donor.approved ? tx("✓ অনুমোদিত") : tx("অনুমোদন করুন")}
      </button>

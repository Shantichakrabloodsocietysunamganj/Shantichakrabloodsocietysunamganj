...
import { BadgeCheck, Banknote, Check, ClipboardList, CreditCard, Droplets, HandHeart, Handshake, Heart, Hospital, Laptop, MapPin, Megaphone, Phone, Siren, type LucideIcon } from "@/components/icons";
...
const WHY: { icon: LucideIcon; bn: string; en: string }[] = [
  ...
  { icon: HandHeart, bn: "স্বেচ্ছাসেবক নেটওয়ার্ক — এলাকাভিত্তিক স্বেচ্ছাসেবক পরিচালনা", en: "Volunteer network — area-based volunteer management" },
...
const WHAT_WE_DO: { icon: LucideIcon; bn: string; en: string; desc_bn: string; desc_en: string }[] = [
  ...
  { icon: MapPin, bn: "এলাকায় সক্রিয়", en: "Grassroots Presence", desc_bn: "৪ জেলার প্রতিটি উপজেলায় আমাদের স্বেচ্ছাসেবক ও দাতা নেটওয়ার্ক।", desc_en: "Volunteers & donors in every upazila of 4 districts." },
  { icon: BadgeCheck, bn: "সম্পূর্ণ ফ্রি", en: "100% Free Service", desc_bn: "কোনো অর্থের লেনদেন নেই — পুরোটাই মানবিক, স্বেচ্ছাসেবী।", desc_en: "No money involved — purely humanitarian & voluntary." },
];

const TIMELINE: { icon: LucideIcon; bn: string; en: string }[] = [
  { icon: Banknote, bn: "আপনার অনুদান", en: "Your Donation" },
  { icon: HandHeart, bn: "স্বেচ্ছাসেবক সহায়তা", en: "Volunteer Support" },
      ...
      <div className="container-page">
        ...
        <Reveal>
          <div className="mx-auto max-w-2xl text-center">
            <span className="inline-flex items-center gap-2 rounded-full bg-blood-500/10 px-4 py-1.5 text-xs font-bold uppercase tracking-wide text-blood-600"><Heart className="h-3.5 w-3.5 fill-blood-600" />{en ? "Support Us" : "সহযোগিতা করুন"}</span>
          ...
          {WHAT_WE_DO.map((w, i) => (
            <Reveal key={i} delay={i * 80}>
              <div className="card-hover h-full p-5 text-center">
                <span className="mx-auto flex h-11 w-11 items-center justify-center rounded-2xl bg-brand-50 text-brand-600 ring-1 ring-brand-100"><w.icon className="h-5 w-5" /></span>
        ...
        {hasMethods ? (
              ...
              <Reveal key={i} delay={i * 80}>
                <div className="card-hover group relative flex flex-col overflow-hidden p-5">
                  ...
                  <button onClick={() => copy(m.account_number)} className="mt-2 inline-flex items-center gap-1.5 rounded-lg bg-brand-50 px-3 py-1.5 text-xs font-bold text-brand-700 transition hover:bg-brand-100 dark:bg-white/5">
                    {copied === m.account_number ? (<><Check className="h-3.5 w-3.5 text-emerald-600" />{en ? "Copied!" : "কপি হয়েছে!"}</>) : (<><ClipboardList className="h-3.5 w-3.5" />{en ? "Copy" : "কপি করুন"}</>)}
                  </button>
          ...
          <Reveal>
            <div className="mx-auto mt-10 max-w-md rounded-2xl border border-dashed border-brand-200 bg-brand-50/30 p-8 text-center dark:border-white/10 dark:bg-white/5">
              <span className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-brand-50 text-brand-600 ring-1 ring-brand-100"><CreditCard className="h-6 w-6" /></span>
              ...
              <Link href="/contact" className="btn-primary mt-4"><Phone className="mr-1.5 inline h-4 w-4" />{en ? "Contact Us" : "যোগাযোগ করুন"}</Link>
            </div>
          </Reveal>
        )}
            ...
            {WHY.map((w, i) => (
              <Reveal key={i} delay={i * 60}>
                <div className="card-hover flex items-start gap-3 p-4">
                  <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-brand-50 text-brand-600 ring-1 ring-brand-100"><w.icon className="h-4 w-4" /></span>
              ...
              {TIMELINE.map((t, i) => (
                <div key={i} className="flex flex-1 flex-col items-center gap-2">
                  <div className="relative">
                    <span className="flex h-14 w-14 items-center justify-center rounded-full bg-white/15 ring-2 ring-white/25 backdrop-blur-sm"><t.icon className="h-6 w-6" /></span>

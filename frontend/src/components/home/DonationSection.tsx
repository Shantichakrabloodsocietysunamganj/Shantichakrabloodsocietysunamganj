...
import { Check, Copy, Droplets, Handshake, Heart, HeartHandshake, Hospital, Laptop, MapPin, Megaphone, Phone, Siren, Users, Wallet, BadgePercent } from "lucide-react";
...
const WHY = [
  { icon: Siren, bn: "জরুরি রক্ত সমন্বয় — যেকোনো সময় দাতা খুঁজে রোগীর কাছে পৌঁছে দেওয়া", en: "Emergency blood coordination — finding donors anytime" },
  { icon: Users, bn: "স্বেচ্ছাসেবক নেটওয়ার্ক — এলাকাভিত্তিক স্বেচ্ছাসেবক পরিচালনা", en: "Volunteer network — area-based volunteer management" },
  { icon: Megaphone, bn: "সচেতনতামূলক প্রচার — নিরাপদ রক্তদানে উৎসাহিত করা", en: "Awareness campaigns — promoting safe blood donation" },
  { icon: Laptop, bn: "প্রযুক্তি — ওয়েবসাইট, অ্যাপ ও ডেটাবেস পরিচালনা", en: "Technology — website, database & platform maintenance" },
  { icon: Droplets, bn: "রক্তদান শিবির — নিয়মিত ফ্রি রক্ত গ্রুপ নির্ধারণ ও শিবির", en: "Blood donation camps — free grouping & camps" },
  { icon: Handshake, bn: "সামাজিক সেবা — দরিদ্র রোগীদের বিনামূল্যে সহায়তা", en: "Community outreach — free help for needy patients" },
];

const WHAT_WE_DO = [
  { icon: Droplets, bn: "রক্তদাতা যুক্ত করি", en: "Connect Blood Donors", desc_bn: "সিলেট বিভাগ জুড়ে নিবন্ধিত দাতাদের এক ছাদে — রোগীর প্রয়োজনে তাৎক্ষণিক যুক্ত করি।", desc_en: "Registered donors across Sylhet — instantly connected to patients in need." },
  { icon: Siren, bn: "জরুরি সেবা ২৪/৭", en: "24/7 Emergency Service", desc_bn: "মাঝরাতেও রক্তের প্রয়োজন হলে আমরা পাশে — কোনো বিরতি নেই।", desc_en: "Blood needed at midnight? We're there — no breaks." },
  { icon: MapPin, bn: "এলাকায় সক্রিয়", en: "Grassroots Presence", desc_bn: "সিলেট বিভাগের ৪ জেলা ও বিভিন্ন উপজেলায় আমাদের স্বেচ্ছাসেবী রক্তদাতা নেটওয়ার্ক বিস্তার লাভ করছে।", desc_en: "Growing volunteer & donor network across 4 districts of Sylhet." },
  { icon: BadgePercent, bn: "সম্পূর্ণ ফ্রি", en: "100% Free Service", desc_bn: "কোনো অর্থের লেনদেন নেই — পুরোটাই মানবিক, স্বেচ্ছাসেবী।", desc_en: "No money involved — purely humanitarian & voluntary." },
];

const TIMELINE = [
  { icon: Wallet, bn: "আপনার অনুদান", en: "Your Donation" },
  { icon: Users, bn: "স্বেচ্ছাসেবক সহায়তা", en: "Volunteer Support" },
  { icon: Droplets, bn: "রক্ত সমন্বয়", en: "Blood Coordination" },
  { icon: Hospital, bn: "রোগী রক্ত পান", en: "Patient Receives Blood" },
      ...
      <div className="container-page">
        ...
        <Reveal>
          <div className="mx-auto max-w-2xl text-center">
            <span className="inline-flex items-center gap-2 rounded-full bg-blood-500/10 px-4 py-1.5 text-xs font-bold uppercase tracking-wide text-blood-600"><Heart className="h-3.5 w-3.5 fill-current" /> {en ? "Support Us" : "সহযোগিতা করুন"}</span>
          ...
          {WHAT_WE_DO.map((w, i) => (
            <Reveal key={i} delay={i * 80}>
              <div className="card-hover h-full p-5 text-center">
                <span className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-blood-500/10 text-blood-600"><w.icon className="h-6 w-6" strokeWidth={1.8} /></span>
        ...
        {hasMethods ? (
              ...
              <Reveal key={i} delay={i * 80}>
                <div className="card-hover group relative flex flex-col overflow-hidden p-5">
                  ...
                  <button onClick={() => copy(m.account_number)} className="mt-2 inline-flex items-center gap-1.5 rounded-lg bg-brand-50 px-3 py-1.5 text-xs font-bold text-brand-700 transition hover:bg-brand-100 dark:bg-white/5">
                    {copied === m.account_number ? <><Check className="h-3.5 w-3.5" /> {en ? "Copied!" : "কপি হয়েছে!"}</> : <><Copy className="h-3.5 w-3.5" /> {en ? "Copy" : "কপি করুন"}</>}
                  </button>
          ...
          <Reveal>
            <div className="mx-auto mt-10 max-w-md rounded-2xl border border-dashed border-brand-200 bg-brand-50/30 p-8 text-center dark:border-white/10 dark:bg-white/5">
              <span className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-white text-blood-600 shadow-sm"><HeartHandshake className="h-6 w-6" strokeWidth={1.8} /></span>
              <p className="mt-3 font-medium text-ink">{en ? "Want to support us directly?" : "সরাসরি সহযোগিতা করতে চান?"}</p>
              <p className="mt-1 text-sm text-ink/55">{en ? "Call or WhatsApp us — our volunteers will guide you." : "কল বা WhatsApp করুন — আমাদের স্বেচ্ছাসেবকরা আপনাকে গাইড করবেন।"}</p>
              <div className="mt-4 flex flex-wrap justify-center gap-2.5">
                <a href={`tel:${site.phone}`} className="btn-primary inline-flex items-center gap-2"><Phone className="h-4 w-4" /> {en ? "Call Us" : "কল করুন"}</a>
                <a href={site.whatsapp} target="_blank" rel="noreferrer" className="btn-outline inline-flex items-center gap-2">{en ? "WhatsApp" : "WhatsApp"}</a>
              </div>
            </div>
          </Reveal>
        )}
            ...
            {WHY.map((w, i) => (
              <Reveal key={i} delay={i * 60}>
                <div className="card-hover flex items-start gap-3 p-4">
                  <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-brand-50 text-brand-600"><w.icon className="h-5 w-5" strokeWidth={1.8} /></span>
                  <p className="pt-1 text-sm font-medium text-ink/70">{en ? w.en : w.bn}</p>
                </div>
              </Reveal>
            ))}
          </div>
              ...
              {TIMELINE.map((t, i) => (
                <div key={i} className="flex flex-1 flex-col items-center gap-2">
                  <div className="relative">
                    <span className="flex h-14 w-14 items-center justify-center rounded-full bg-white/15 ring-2 ring-white/25 backdrop-blur-sm"><t.icon className="h-6 w-6" strokeWidth={1.8} /></span>

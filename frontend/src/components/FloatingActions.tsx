...
export default function FloatingActions() {
  ...
  return (
    <div className="fixed bottom-6 left-4 z-40 flex flex-col gap-2.5">
      ...
      <Link
        href="/request-blood"
        aria-label={tx("জরুরি রক্তের অনুরোধ")}
      ...
      >
        ...
        <span className="pointer-events-none absolute left-14 whitespace-nowrap rounded-lg bg-white px-2 py-1 text-[11px] font-bold text-blood-700 opacity-0 shadow transition-opacity group-hover:opacity-100 dark:bg-slate-800 dark:text-blood-300">{tx("জরুরি রক্ত")}</span>
      </Link>
      {/* Call */}
      <a
        href={`tel:${site.phone}`}
        aria-label={tx("কল করুন")}
      ...
      >
        ...
        <span className="pointer-events-none absolute left-14 whitespace-nowrap rounded-lg bg-white px-2 py-1 text-[11px] font-bold text-brand-700 opacity-0 shadow transition-opacity group-hover:opacity-100 dark:bg-slate-800 dark:text-brand-300">{tx("কল করুন")}</span>
      </a>
      {/* WhatsApp */}
      <a
        href={site.whatsapp}
